...
function Find-SightlineAndroidDevice {
    <#
        Resolves an enrolled Android device by name or serial.

        Scoped to Android so a name shared with a Windows machine cannot return
        the wrong record.
    #>
    ...
    foreach ($field in @('deviceName', 'serialNumber')) {
        try {
            $result = Invoke-SightlineGraphRequest `
                -Uri "$base/managedDevices?`$filter=operatingSystem eq 'Android' and $field eq '$escaped'&`$select=$select"
            $hits = @($result.value)

            if ($hits.Count -gt 1) {
                $list = (@($hits | ForEach-Object { "$($_.deviceName) [$($_.serialNumber)]" }) -join '; ')
                return [pscustomobject]@{ Device = $null; Problem = "'$Query' matched $($hits.Count) Android devices: $list. Use a serial number." }
            }
    ...
    # A device managed only by app protection policy has no managed device
    # record at all. Saying "not found" would send an admin looking for a typo.
    return [pscustomobject]@{
        Device  = $null
        Problem = "No enrolled Android device matched '$Query', by name or serial number. " +
...
function Get-SightlineAndroidEnrolment {
    <#
        The enrolment profile a device provisioned through, and the token that
        allowed it.

        Device Owner profiles apply to corporate-owned enrolments only. A
        personally owned work profile device enrols without one, so an absent
        profile is a fact about the enrolment type rather than missing data.

        The profile object also carries tokenValue, qrCodeContent and
        wifiPassword. Those are live enrolment credentials - anyone holding one
        can enrol a device into the tenant - so they are never read here.
    #>
    param(
    ...
    $names = $Device.PSObject.Properties.Name
    $ownership = if ($names -contains 'managedDeviceOwnerType') { [string]$Device.managedDeviceOwnerType } else { '' }
    $agent     = if ($names -contains 'managementAgent') { [string]$Device.managementAgent } else { '' }
    ...
    # Personally owned work profile: managed, but not through a Device Owner
    # profile. Labelling it unmanaged would contradict the policies listed below.
    if ($agent -eq 'googleCloudDevicePolicyController' -and $ownership -eq 'personal') {
        $result.Mode = 'Personally owned - work profile (BYOD)'
        $result.ProfileApplies = $false
        $result.Note = 'Device Owner enrolment profiles and enrolment tokens do not apply to this enrolment type, so those fields are absent by design. The device is fully managed inside its work profile.'
        return $result
    }

    if (-not $profileName) {
        $result.Mode = if ($ownership) { "Corporate-owned ($ownership)" } else { 'Unknown' }
        $result.ProfileApplies = $false
        $result.Note = 'No enrolment profile is recorded against this device, so the provisioning route cannot be determined from here.'
        return $result
    }

    $match = @($Profiles | Where-Object {
        $_.PSObject.Properties.Name -contains 'displayName' -and
        ([string]$_.displayName) -eq $profileName
    })

    if ($match.Count -eq 0) {
        $result.Mode = 'Unknown'
        $result.Note = "The device records enrolment profile '$profileName', but no profile of that name exists now. It was probably deleted after the device enrolled."
        return $result
    }
    ...
    $result.Mode = switch ([string]$profile.enrollmentMode) {
        'corporateOwnedDedicatedDevice'          { 'Corporate-owned dedicated device' }
        'corporateOwnedFullyManaged'             { 'Corporate-owned fully managed' }
        'corporateOwnedWorkProfile'              { 'Corporate-owned work profile' }
        'corporateOwnedAOSPUserlessDevice'       { 'AOSP userless device' }
        'corporateOwnedAOSPUserAssociatedDevice' { 'AOSP user-associated device' }
        default { if ($pn -contains 'enrollmentMode') { [string]$profile.enrollmentMode } else { 'Unknown' } }
    }

    $result.TokenType = switch ([string]$profile.enrollmentTokenType) {
        'corporateOwnedDedicatedDeviceWithAzureADSharedMode' { 'Dedicated device with shared mode' }
        'deviceStaging' { 'Device staging' }
        'default'       { 'Standard' }
        default { if ($pn -contains 'enrollmentTokenType') { [string]$profile.enrollmentTokenType } else { '' } }
    }

    if ($pn -contains 'tokenCreationDateTime')   { $result.TokenCreated  = [string]$profile.tokenCreationDateTime }
    if ($pn -contains 'enrollmentTokenUsageCount') { $result.TokenUsage  = [string]$profile.enrollmentTokenUsageCount }
    if ($pn -contains 'enrolledDeviceCount')     { $result.EnrolledCount = [string]$profile.enrolledDeviceCount }

    if ($pn -contains 'tokenExpirationDateTime' -and $profile.tokenExpirationDateTime) {
        $result.TokenExpires = [string]$profile.tokenExpirationDateTime
        try {
            $result.DaysToExpiry = [int]([datetime]$profile.tokenExpirationDateTime - (Get-Date)).TotalDays
        }
        catch { }
    }

    if ($pn -contains 'isTeamsDeviceProfile' -and $profile.isTeamsDeviceProfile) {
        $result.Note = 'This is a Teams device profile.'
    }
...
'@
    ...
    [void]$h.Append('<h1>Android device journey</h1><p class="sub">' + (& $e $Serial))
    ...
    if ($device) {
        ...
        $facts = @()
        if ($device.EnrolmentMode)    { $facts += @{ k = 'Enrolment';      v = $device.EnrolmentMode } }
        ...
        if ($device.Enrolled)         { $facts += @{ k = 'Enrolled';       v = $device.Enrolled } }
    ...
    elseif ($device -and $device.EnrolmentProfile -and $device.EnrolmentProfile -ne 'Not applicable') {
        ...
        if ($device.TokenType) { $line += ', ' + (& $e $device.TokenType) + ' token' }
        if ($device.TokenExpires) {
            $line += '. Token expires ' + (& $e $device.TokenExpires)
            if ($device.TokenUsage) { $line += ', used ' + (& $e $device.TokenUsage) + ' time(s)' }
        }
...
function Invoke-Tool {
    ...
    # --- Resolve each device ---------------------------------------------------
    & $ReportProgress 4 'Listing Android enrolment profiles'

    $profiles = @()
    try {
        # Only the descriptive fields. tokenValue, qrCodeContent and
        # wifiPassword are live enrolment credentials and are never read.
        $listed = Get-SightlineGraphCollection `
            -Uri "https://graph.microsoft.com/beta/deviceManagement/androidDeviceOwnerEnrollmentProfiles?`$select=id,displayName,description,enrollmentMode,enrollmentTokenType,tokenCreationDateTime,tokenExpirationDateTime,enrollmentTokenUsageCount,enrolledDeviceCount,isTeamsDeviceProfile"
        $profiles = @($listed.Items)
        $coverage.Add([pscustomobject]@{
            Source = 'androidDeviceOwnerEnrollmentProfiles'; Count = $listed.Count
            Complete = $listed.Complete; Failure = $listed.Failure
        })
    }
    catch {
        $warnings.Add("Android enrolment profiles could not be read - $($_.Exception.Message). Enrolment detail will be missing.")
    }
    ...
    foreach ($query in $serials) {
        $deviceNo++
        & $ReportProgress (8 + $deviceNo * 5) "Finding $query"

        $lookup = Find-SightlineAndroidDevice -Query $query
        ...
        $enrolmentOf[$serial] = Get-SightlineAndroidEnrolment -Device $device -Profiles $profiles
        ...
        $expiry = $enrolmentOf[$serial].DaysToExpiry
        if ($null -ne $expiry) {
            if ($expiry -lt 0) {
                $warnings.Add("$serial : the enrolment token on '$($enrolmentOf[$serial].ProfileName)' expired $([math]::Abs($expiry)) day(s) ago. Existing devices are unaffected, but no new device can enrol through it.")
            }
            elseif ($expiry -le 30) {
                $warnings.Add("$serial : the enrolment token on '$($enrolmentOf[$serial].ProfileName)' expires in $expiry day(s).")
            }
        }
    ...
    $coverage.Add([pscustomobject]@{
        Source = 'managedDevices (Android, by name or serial)'; Count = $found.Count
        Complete = $true; Failure = $null
    })

    if ($found.Count -eq 2) {
        $modes = @($serials | ForEach-Object { $enrolmentOf[$_].Mode } | Select-Object -Unique)
        if ($modes.Count -gt 1) {
            $warnings.Add("The two devices enrolled differently: $($modes -join ' and '). Enrolment mode governs what policy can do on Android, so that alone may explain the differences below.")
        }
    }
    ...
    foreach ($device in $found) {
        ...
        $deviceRows.Add([pscustomobject]@{
            ...
            EnrolmentMode  = $enrol.Mode
            EnrolmentProfile = if ($enrol.ProfileApplies) { $enrol.ProfileName } else { 'Not applicable' }
            TokenType      = $enrol.TokenType
            TokenExpires   = $enrol.TokenExpires
            TokenCreated   = $enrol.TokenCreated
            TokenUsage     = $enrol.TokenUsage
            ...
            PatchLevel     = if ($dn -contains 'androidSecurityPatchLevel') { $device.androidSecurityPatchLevel } else { '' }
    ...
    # Android has no group tag, so two devices are always comparable.
    $tagsDiffer = $false
