...
$ToolId      = 'macos-journey'
$ToolVersion = '1.0.0'

function Find-SightlineMacDevice {
    ...
    $select  = 'id,deviceName,serialNumber,azureADDeviceId,operatingSystem,osVersion,' +
               ...
               'isSupervised,isEncrypted,azureADRegistered'

    foreach ($field in @('deviceName', 'serialNumber')) {
        try {
            $result = Invoke-SightlineGraphRequest `
                -Uri "$base/managedDevices?`$filter=operatingSystem eq 'macOS' and $field eq '$escaped'&`$select=$select"
            $hits = @($result.value)

            if ($hits.Count -gt 1) {
                $list = (@($hits | ForEach-Object { "$($_.deviceName) [$($_.serialNumber)]" }) -join '; ')
                return [pscustomobject]@{ Device = $null; Problem = "'$Query' matched $($hits.Count) Macs: $list. Use a serial number." }
            }
    ...
    return [pscustomobject]@{
        Device  = $null
        Problem = "No enrolled Mac matched '$Query', by name or serial number. " +
...
function Get-SightlineMacEnrolment {
    <#
        How a Mac came to be enrolled.

        Intune recognises four routes: Automated Device Enrolment, BYOD device
        enrolment (which Intune also calls user-approved), direct enrolment, and
        the device enrolment manager.

        deviceEnrollmentType cannot always tell them apart. Microsoft documents
        appleBulkWithUser as covering DEP and Apple Configurator, and
        appleBulkWithoutUser as covering DEP, Apple Configurator and Mobile
        Config - so on those two values the route is genuinely ambiguous and is
        reported as such rather than guessed. The newer ACME values do
        distinguish, and the beta endpoint this tool queries may return them.
    #>
    param(
    ...
    switch ($type) {
        'appleBulkWithUser' {
            $result.Route = 'Apple bulk enrolment, with user affinity'
            $result.Ambiguous = $true
            $result.IsAde = $true
        }
        'appleBulkWithoutUser' {
            $result.Route = 'Apple bulk enrolment, without user affinity'
            $result.Ambiguous = $true
            $result.IsAde = $true
        }
        'appleACMEDEPUserless'          { $result.Route = 'Automated Device Enrolment, without user affinity'; $result.IsAde = $true }
        'appleACMEDEPUDACompanyPortal'  { $result.Route = 'Automated Device Enrolment, Company Portal authentication'; $result.IsAde = $true }
        'appleACMEDEPUDASetupAsstLegacy'{ $result.Route = 'Automated Device Enrolment, Setup Assistant (legacy)'; $result.IsAde = $true }
        'appleACMEDEPUDAModernAuth'     { $result.Route = 'Automated Device Enrolment, Setup Assistant with modern authentication'; $result.IsAde = $true }
        'appleACMEBasicBYOD'            { $result.Route = 'Device enrolment (BYOD, user approved)' }
        'userEnrollment'                { $result.Route = 'Device enrolment through Company Portal (BYOD, user approved)' }
        'deviceEnrollmentManager'       { $result.Route = 'Device enrolment manager' }
        'appleUserEnrollment'           { $result.Route = 'User enrolment' }
        'appleAccountDrivenUserEnrollment' { $result.Route = 'Account-driven user enrolment' }
        default {
            # The enum is documented as open: new values appear without notice.
            $result.Route = if ($type) { $type } else { 'Unknown' }
        }
    }

    if ($result.Ambiguous) {
        $result.Note = 'Intune reports this as a bulk Apple enrolment, which covers Automated Device Enrolment, Apple Configurator and direct enrolment alike. The exact route cannot be determined from the device record.'
    }
    ...
    if (-not $profileName) { return $result }
    ...
    if (-not $result.Note) {
        $result.Note = "The device records enrolment profile '$profileName', but no macOS Automated Device Enrolment profile of that name exists now. It was probably renamed or deleted after the device enrolled."
    }
    return $result
}

function Get-SightlineMacAppLicensing {
    # macOS app types differ from iOS, so the licensing column needs its own
    # mapping. Read from the app's type, never inferred from a missing token.
    ...
    switch -Wildcard ($type) {
        '*macOSVppApp'            { 'VPP (organisation not reported)' }
        '*macOSPkgApp'            { 'PKG upload' }
        '*macOSDmgApp'            { 'DMG upload' }
        '*macOSLobApp'            { 'Line of business' }
        '*macOSMicrosoftEdgeApp'  { 'Microsoft Edge (managed)' }
        '*macOSMicrosoftDefenderApp' { 'Microsoft Defender (managed)' }
        '*macOSOfficeSuiteApp'    { 'Microsoft 365 apps (managed)' }
        '*macOSWebClip'           { 'Web clip' }
        '*macOsVppApp'            { 'VPP (organisation not reported)' }
        ''                        { '' }
...
'@
    ...
    [void]$h.Append('<h1>macOS device journey</h1><p class="sub">' + (& $e $Serial))
    ...
    if ($device) {
        ...
        if ($device.EntraRegistered)  { $facts += @{ k = 'In Entra';     v = $device.EntraRegistered } }
        if ($device.Supervised)       { $facts += @{ k = 'Supervision';  v = $device.Supervised } }
        if ($device.FileVault)        { $facts += @{ k = 'Encryption';   v = $device.FileVault } }
    ...
    # The most consequential fact about a Mac, and the one an admin is least
    # likely to expect: managed, but invisible to group targeting.
    if ($device -and $device.EntraRegistered -eq 'No') {
        [void]$h.Append('<p class="tagline" style="border-left-color:var(--bad)">This Mac has <strong>no Microsoft Entra device object</strong>. It is enrolled and managed, but nothing targeted at a group can reach it - only assignments to all devices apply. This is normal for a Mac enrolled through Setup Assistant (legacy), which authenticates against the Apple token rather than Entra.</p>')
    }
...
function Invoke-Tool {
    ...
    foreach ($query in $serials) {
        ...
        $lookup = Find-SightlineMacDevice -Query $query
        ...
        $enrolmentOf[$serial] = Get-SightlineMacEnrolment -Device $device -Tokens $tokens

        # A Mac enrolled through Setup Assistant legacy is authenticated by the
        # Apple token and is never registered in Entra. That is a supported
        # state, not a failure: the device is managed, but nothing targeted at a
        # group can reach it. Reporting that is the answer.
        if (-not ($names -contains 'azureADDeviceId') -or -not $device.azureADDeviceId) {
            $unregistered.Add($serial)
            $originOf[$serial] = [pscustomobject]@{
                DirectoryId = ''; DisplayName = ''; JoinType = 'Not registered in Microsoft Entra'
                TrustType = ''; ProfileType = ''; RegisteredOn = ''; JoinedBy = ''; Problem = $null
            }
        }
        else {
    ...
    $coverage.Add([pscustomobject]@{
        Source = 'managedDevices (macOS, by name or serial)'; Count = $found.Count
        Complete = $true; Failure = $null
    })

    foreach ($serial in $unregistered) {
        $warnings.Add("$serial is enrolled and managed but has no Microsoft Entra device object. Nothing targeted at a group can reach it - only assignments to all devices apply. This is normal for a Mac enrolled through Setup Assistant (legacy), which authenticates against the Apple token rather than Entra.")
    }

    if ($found.Count -eq 2) {
        $routes = @($serials | ForEach-Object { $enrolmentOf[$_].Route } | Select-Object -Unique)
        if ($routes.Count -gt 1) {
            $warnings.Add("The two Macs enrolled by different routes: $($routes -join ' and '). Enrolment route governs supervision and Entra registration, so that alone may explain the differences below.")
        }
    }
    ...
    foreach ($device in $found) {
        ...
        $deviceRows.Add([pscustomobject]@{
            ...
            RouteCertain   = if ($enrol.Ambiguous) { 'No - see note' } else { 'Yes' }
            ...
            EntraRegistered = if ($origin.JoinType -eq 'Not registered in Microsoft Entra') { 'No' } else { 'Yes' }
            FileVault      = if ($dn -contains 'isEncrypted') { if ($device.isEncrypted) { 'FileVault on' } else { 'FileVault off' } } else { 'Unknown' }
        ...
        foreach ($policy in $listed.Items) {
            ...
            foreach ($assignment in $policyAssignments.Items) {
                ...
                $licensing = if ($surface.Kind -eq 'Application') {
                    Get-SightlineMacAppLicensing -App $policy
                } else { '' }
    ...
    function Write-Journey {
        ...
        if ($writeHtml) {
            foreach ($serial in $Serials) {
                ...
                $htmlName = 'macos-journey-' + ($serial -replace '[^A-Za-z0-9\-]', '-') + '.html'
    ...
    if ($tagsDiffer) {
        ...
        foreach ($serial in $serials) {
            ...
            Write-Journey -Serials @($serial) -BaseName "macos-journey-$safe"
        }
    }
    else {
        $extra = @()
        ...
        Write-Journey -Serials $serials -BaseName "macos-journey-$safe" -Compare:($found.Count -eq 2) -ExtraSheets $extra
    }
